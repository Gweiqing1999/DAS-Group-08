...
The data contains `r nrow(group_08)` observations and 7 variables. The variables year, length, budget, votes, and rating are numerical variables, and genre is a categorical variable. Table \ref{tab:head} shows the first 6 rows of the table. It is also worth of note that the length of some of the films is not recorded.
...
From the summary statistics in table \ref{tab:sums}, we can see that the year of release of the films is between `r min(imdb_group_08$year)` to `r max(imdb_group_08$year)`. The minimum budget for film in the data set is `r min(imdb_group_08$budget)` million while the maximum is `r max(imdb_group_08$budget)` million.

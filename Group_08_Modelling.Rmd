...
$$\log(\frac{\hat{p}}{1-\hat{p}}) = -1.84 + 0 \cdot \mathrm{II}_{genre}({Action})$$
$$\qquad \quad \; \log(\frac{\hat{p}}{1-\hat{p}}) = -1.84 + 2.55 \cdot \mathrm{II}_{genre}({Animation})$$
$$...$$
where $\mathrm{II}_{genre}({\cdot})$ is an indicator function, that takes a specific movie genre as input and returns 1 if the input variable matches the genre, and 0 otherwise.
...
$$g(\hat{p_i}) = log(\frac{\hat{p_i}}{1-\hat{p_i}}) = -23.86 + 0.01 \cdot year_i - 0.06 \cdot length_i + 0.51 \cdot budget_i + \hat{\beta_4} \cdot \mathrm{II}_{genre}({\cdot})$$
